...
async def confirm_withdrawal_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"Admin {admin_user.id} confirming withdrawal ID: {withdrawal_id}")
        
        # Check if already processed by looking at message text
        if "✅ PAID by" in query.message.text:
            await query.answer("This withdrawal has already been processed.", show_alert=True)
            return

        # Confirm withdrawal in database
        ...
        user_message = "✅ Your withdrawal has been successfully released. Check your wallet 💰"
        ...
        try:
            ...
            logger.info(f"Updated admin message for withdrawal ID {withdrawal_id}")
        except Exception as e:
            ...
            try:
                ...
                logger.error(f"Failed to remove keyboard: {e2}")

        await query.answer("✅ Payment confirmed successfully!", show_alert=True)
        
    except (IndexError, ValueError) as e:
        logger.error(f"Invalid withdrawal ID in callback data: {query.data} - {e}")
        await query.answer("Invalid request data.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in confirm_withdrawal_handler: {e}", exc_info=True)
        await query.answer(f"Error processing payment: {str(e)}", show_alert=True)
...
def get_admin_handlers():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            AdminState.ADD_COUNTRY_STEP_2: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_add_country_name)],
            AdminState.ADD_COUNTRY_STEP_3: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_add_country_flag)],
            AdminState.ADD_COUNTRY_STEP_4: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_add_country_price_ok)],
            AdminState.ADD_COUNTRY_STEP_5: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_add_country_price_restricted)],
            AdminState.ADD_COUNTRY_STEP_6: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_add_country_time)],
            AdminState.ADD_COUNTRY_STEP_7: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_add_country_capacity)],
...
async def handle_add_country_code(update, context):
    ...
    return AdminState.ADD_COUNTRY_STEP_2

async def handle_add_country_name(update, context):
    name = update.message.text.strip()
    context.user_data['new_country_name'] = name
    await update.message.reply_text("Step 3/7: Enter country flag emoji:")
    return AdminState.ADD_COUNTRY_STEP_3

async def handle_add_country_flag(update, context):
    flag = update.message.text.strip()
    context.user_data['new_country_flag'] = flag
    await update.message.reply_text("Step 4/7: Enter price for OK status (e.g., 0.50):")
    return AdminState.ADD_COUNTRY_STEP_4

async def handle_add_country_price_ok(update, context):
    try:
        price_ok = float(update.message.text.strip())
        context.user_data['new_country_price_ok'] = price_ok
        await update.message.reply_text("Step 5/7: Enter price for restricted status (e.g., 0.10):")
        return AdminState.ADD_COUNTRY_STEP_5
    except ValueError:
        await update.message.reply_text("❌ Invalid number. Please enter a valid price:")
        return AdminState.ADD_COUNTRY_STEP_4

async def handle_add_country_price_restricted(update, context):
    try:
        price_restricted = float(update.message.text.strip())
        context.user_data['new_country_price_restricted'] = price_restricted
        await update.message.reply_text("Step 6/7: Enter confirmation time in seconds (e.g., 600):")
        return AdminState.ADD_COUNTRY_STEP_6
    except ValueError:
        await update.message.reply_text("❌ Invalid number. Please enter a valid price:")
        return AdminState.ADD_COUNTRY_STEP_5

async def handle_add_country_time(update, context):
    try:
        time_seconds = int(update.message.text.strip())
        context.user_data['new_country_time'] = time_seconds
        await update.message.reply_text("Step 7/7: Enter capacity (-1 for unlimited, or a number like 100):")
        return AdminState.ADD_COUNTRY_STEP_7
    except ValueError:
        await update.message.reply_text("❌ Invalid number. Please enter time in seconds:")
        return AdminState.ADD_COUNTRY_STEP_6

async def handle_add_country_capacity(update, context):
    try:
        capacity = int(update.message.text.strip())
        
        # Get all the collected data
        code = context.user_data.get('new_country_code')
        name = context.user_data.get('new_country_name')
        flag = context.user_data.get('new_country_flag')
        price_ok = context.user_data.get('new_country_price_ok')
        price_restricted = context.user_data.get('new_country_price_restricted')
        time_seconds = context.user_data.get('new_country_time')
        
        # Add the country to database
        database.add_country(code, name, flag, time_seconds, capacity, price_ok, price_restricted, None, 'True', 'False')
        context.bot_data['countries_config'] = database.get_countries_config()
        ...
        await update.message.reply_text(
            ...
            f"📛 Name: {name}\n"
            f"{flag} Flag: {flag}\n"
            f"💰 OK Price: ${price_ok:.2f}\n"
            f"⚠️ Restricted Price: ${price_restricted:.2f}\n"
            f"⏱️ Time: {time_seconds}s\n"
            f"📦 Capacity: {'Unlimited' if capacity == -1 else capacity}"
        )
        ...
        await update.message.reply_text("❌ Invalid number. Please enter capacity (-1 for unlimited):")
        return AdminState.ADD_COUNTRY_STEP_7
