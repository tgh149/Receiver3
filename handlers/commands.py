...
async def handle_withdrawal_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not address:
            await update.message.reply_text("❌ Please enter a wallet address.")
            return
        ...
        if withdrawal_id:
            ...
            try:
                # Send to support channel (https://t.me/RAESUPPORT)
                support_channel = "@RAESUPPORT"
                user = update.effective_user

                admin_text = f"💸 New Withdrawal Request\n\n"
                admin_text += f"👤 User: @{user.username or 'N/A'} ({user.id})\n"
                admin_text += f"💰 Amount: ${amount:.2f}\n"
                admin_text += f"📬 Address: {address}\n"
                ...
                logger.info(f"Sent withdrawal notification to {support_channel} for request ID {withdrawal_id}")
            except Exception as e:
                logger.error(f"Failed to notify admin about withdrawal: {e}")
                # Try to send to admin users as fallback
                try:
                    ...
                    for admin in admins:
                        ...
                        logger.info(f"Sent withdrawal notification to admin {admin['telegram_id']} as fallback")
                except Exception as e2:
                    logger.error(f"Failed to notify admins as fallback: {e2}")
        else:
...
async def on_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # If not handled above and user is not an admin, forward to support
        if not database.is_admin(user_id):
            await proxy_chat.forward_to_admin(update, context)
        # For admins, don't send any response for unhandled text messages
